...
if (isset($_SESSION['mobile']) && isset($_SESSION['role']) && $_SESSION['role'] === 'member') {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($mobile && $password) {
    ...
    if ($member) {
      ...
      if (!empty($member['sub_date']) && $member['sub_date'] < $currentDate) {
        ...
        if ($password === $member['password'] || password_verify($password, $member['password'])) {
          // Store all required values in session
          $_SESSION['name'] = $member['name'];
          $_SESSION['mobile'] = $member['mobile'];  
          $_SESSION['region'] = $member['region'];  
          $_SESSION['chapter'] = $member['chapter'];
          $_SESSION['powerteam'] = $member['powerteam'];
...
<head>
  ...
  <title>Member Login - Power Team</title>
...
<body class="min-h-screen bg-gradient-to-br from-gray-50 via-red-50 to-gray-100 font-['Roboto'] flex items-center justify-center p-4">
  ...
  <div class="relative w-full max-w-md px-4 sm:px-0">
      ...
      <div class="bg-white rounded-2xl shadow-2xl overflow-hidden animate-slideIn">
          <!-- Header -->
          <div class="bg-gradient-to-r from-red-400 to-red-500 p-6 sm:p-8 text-white text-center relative overflow-hidden">
              ...
              <div class="relative z-10">
                  <div class="w-16 h-16 sm:w-20 sm:h-20 bg-white rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4 shadow-lg">
                      <i class="fas fa-user text-2xl sm:text-3xl text-red-400"></i>
                  </div>
                  <h2 class="text-xl sm:text-2xl font-bold mb-1 sm:mb-2">Member Login</h2>
                  <p class="text-red-100 text-xs sm:text-sm">Access Your Portal</p>
              </div>
          </div>
          ...
          <div class="p-6 sm:p-8">
              ...
              <form method="POST" action="" class="space-y-5 sm:space-y-6">
                  ...
                  <button
                      type="submit"
                      class="w-full bg-gradient-to-r from-red-400 to-red-500 hover:from-red-500 hover:to-red-600 text-white font-semibold py-2.5 sm:py-3 text-sm sm:text-base rounded-lg transition-all duration-300 transform hover:scale-[1.02] hover:shadow-lg flex items-center justify-center gap-2">
                      <span>Sign In</span>
