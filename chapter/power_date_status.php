...
// REPLACE with:
$chapterid = (int)($_SESSION['chapter'] ?? 0);
...
$query = "SELECT pd.*, 
          ...
          WHERE m.chapter = :chapterid
          ORDER BY pd.date DESC";
$power_dates_stmt = $conn->prepare($query);
$power_dates_stmt->execute([
    ':chapterid' => $chapterid,
]);
...
function applyFilter() {
    ...
    const tbody = document.getElementById('status-details-tbody');
        ...
        noDataRow.innerHTML = `
            <td colspan="${colspan}" class="px-4 py-8 text-center">
                <div class="flex flex-col items-center gap-2 text-gray-400">
                    ...
                    <span class="text-xs text-gray-300">No records found for the selected status.</span>
                </div>
            </td>`;
