...
$selectedPowerteamId = isset($_GET['powerteam']) && $_GET['powerteam'] !== '' 
    ? (int)$_GET['powerteam'] 
    : $powerteamId; // default from session
    // echo "test".$selectedPowerteamId;
...
$paramsAttendance = [
    ':regionId' => $regionId,
    ':chapterId' => $chapterId,
    ':powerteamId' => $selectedPowerteamId
];
...
$sql = "
    ...
    WHERE m.region = :regionId AND m.chapter = :chapterId  And m.status = 1 AND m.powerteam = :selectedPowerteamId
";
...
$stmt->bindValue(':selectedPowerteamId', $selectedPowerteamId);
    ...
    <div class="max-w-7xl mx-auto">
      ...
      <form method="GET" class="mb-6 flex flex-wrap gap-4 bg-white shadow p-4 rounded">
        ...
        <div>
          ...
          <select name="powerteam" class="mt-1 border rounded px-3 py-2">
            <!-- <option value="">All</option> -->
            <?php foreach ($powerteams as $pt): ?>
              <option value="<?= $pt['id'] ?>" <?= $selectedPowerteamId == $pt['id'] ? 'selected' : '' ?>>
