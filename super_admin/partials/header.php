...
<aside id="sidebar" class="fixed top-0 left-0 h-screen w-64 bg-white shadow-2xl transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out z-40 flex flex-col">
    <div class="bg-gradient-to-br from-red-700 to-red-600 p-6 text-white flex-shrink-0">
        <div class="flex items-center gap-3 mb-2">
            ...
            <div>
                ...
                <p class="text-xs text-red-100">Super Admin</p>
            </div>
        </div>
    </div>
    <nav class="flex-1 overflow-y-auto py-4" style="max-height: calc(100vh - 120px);">
        <div class="px-3 space-y-1">
            ...
            <a href="create_admin.php" class="sidebar-link <?= basename($_SERVER['PHP_SELF']) === 'create_admin.php' ? 'active' : '' ?>">
                <i class="fas fa-user-cog w-5"></i><span>Create Admin</span>
            </a>
            <a href="create_member.php" class="sidebar-link <?= basename($_SERVER['PHP_SELF']) === 'create_member.php' ? 'active' : '' ?>">
                <i class="fas fa-user-plus w-5"></i><span>Create Member</span>
            </a>
            <a href="master_manage.php" class="sidebar-link <?= basename($_SERVER['PHP_SELF']) === 'master_manage.php' ? 'active' : '' ?>">
                <i class="fas fa-cog w-5"></i><span>Master Manage</span>
            </a>
            <a href="subscriptions.php" class="sidebar-link <?= basename($_SERVER['PHP_SELF']) === 'subscriptions.php' ? 'active' : '' ?>">
                <i class="fas fa-star w-5"></i><span>Subscriptions</span>
            </a>
        </div>
...
<style>
    .sidebar-link { display:flex; align-items:center; gap:0.75rem; padding:0.75rem 1rem; border-radius:0.5rem; color:#374151; font-weight:500; transition:all 0.2s; text-decoration:none; white-space:nowrap; }
    .sidebar-link:hover { background-color:#fef2f2; color:#b91c1c; }
    .sidebar-link.active { background-color:#b91c1c; color:white; box-shadow:0 4px 6px -1px rgba(0,0,0,0.1); }
    .sidebar-link.active:hover { background-color:#991b1b; color:white; }
    .sidebar-link i { font-size:1.125rem; width:1.25rem; min-width:1.25rem; text-align:center; flex-shrink:0; }
    .sidebar-link span { flex:1; overflow:visible; white-space:nowrap; }
</style>
